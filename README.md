# TradingAgents Pro 中文研投终端

这是一个面向量化学习与研究的专业研投终端。它把 A 股、美股、港股和全球主要市场的行情、K 线与分时图、在线新闻与公告、基本面快照、本地证据评分、多因子挖掘、多策略融合、12 角色智能体研判、风险控制和历史回测放在同一个工作台中，并同时提供 Web、Docker 与 Windows `.exe` 运行方式。
...
交互原型与设计基础保存在 [TradingAgents Pro Figma 文件](https://www.figma.com/design/vglROiapGSJ05SIsjtEdVx)。
...
- 证券目录：A 股和港股优先读取[新浪财经市场中心](https://vip.stock.finance.sina.com.cn/mkt/)，东方财富作为回退；美股读取 [Nasdaq Trader Symbol Directory](https://www.nasdaqtrader.com/trader.aspx?id=symboldirdefs)。目录在 `%LOCALAPPDATA%\TradingAgentsPro\catalog` 中缓存，并随程序提供可离线检索的压缩快照。
...
```powershell
$env:TRADINGAGENTSPRO_SEC_USER_AGENT = "YourApp/1.0 your-email@example.com"
```
...
$env:TRADINGAGENTS_LLM_MODEL = "填写模型ID"
$env:TRADINGAGENTS_LLM_API_KEY = "填写APIKey"
# 可选：覆盖 OpenAI 兼容地址
$env:TRADINGAGENTS_LLM_BASE_URL = "https://api.openai.com/v1"
```

每个服务也可单独配置，例如 `TRADINGAGENTS_OPENAI_MODEL`、`TRADINGAGENTS_OPENAI_API_KEY`、`TRADINGAGENTS_OPENAI_BASE_URL`；DeepSeek、Qwen、Ollama 分别替换变量名中的 `OPENAI`。程序也会读取 `OPENAI_API_KEY`、`DEEPSEEK_API_KEY` 和 `DASHSCOPE_API_KEY`。
...
```powershell
docker build -t tradingagents-pro .
docker run --rm -p 8765:8765 tradingagents-pro
```
...
```text
dist\TradingAgentsPro\TradingAgentsPro.exe
```

桌面端仅监听随机的 `127.0.0.1` 端口，并用一次性会话令牌保护本地 API。运行日志位于 `%LOCALAPPDATA%\TradingAgentsPro\logs\desktop.log`。

## 启动经典桌面工作台

```powershell
cd D:\codex\quant-starter
.\.venv\Scripts\python.exe trading_agents_app.py
```

默认输出位置：

- 投研报告：`Documents\TradingAgentsCN\research`
- 回测结果：`Documents\QuantStarter\outputs`
- 决策记忆：`Documents\TradingAgentsCN\memory\decision_log.jsonl`
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\Build-Pro-Exe.ps1
```
...
```text
dist\TradingAgentsCN\TradingAgentsCN.exe
```
...
.\.venv\Scripts\python.exe trading_agents_app.py --smoke-test
.\.venv\Scripts\python.exe trading_agents_app.py --data-smoke-test
...
```text
trading_agents_app.py              经典 Tkinter 桌面研究与回测入口
